...
import { ArrowDown2, Check, TickCircle } from "iconsax-react";
...
const BaseSelectInput = (props: MyComponentProps) => {
  ...
  return (
    <div
      ...
      } bg-White dark:bg-Nero ${props.square ? "rounded-2xl " : "rounded-full"}
      `}
    ...
    >
      <div
        className={
          "border-2 dark:border-Dark-Grey " +
          (props.square ? "rounded-2xl " : "rounded-full ") +
          classes.selectInput
        }
      ...
      {showOptions && (
        <div className={classes.optionsBox + " bg-white dark:bg-Nero"}>
          {props.options?.map((item, index) => {
            return (
              ...
              >
                <span className="p-2.5 text-nowrap">{item.title}</span>
                {item.value === selectedItem.value && <TickCircle />}
              </div>
            );
          })}
        </div>
